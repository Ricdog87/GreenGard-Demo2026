...
export function Cursor() {
  ...
  useEffect(() => {
    // Disable on coarse pointers (touch) — matches md+ desktop expectation.
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      if (interactive) {
        const c = interactive.dataset.cursor as 'hover' | 'view';
        setVariant(c === 'view' ? 'view' : 'hover');
      } else {
        setVariant('default');
      }
    };
  ...
  const size =
    variant === 'view' ? 64 : variant === 'hover' ? 48 : 8;
  ...
  return (
    <div
      ref={dotRef}
      aria-hidden
      className="pointer-events-none fixed left-0 top-0 z-[80] -translate-x-1/2 -translate-y-1/2 rounded-full transition-[width,height] duration-200 ease-out grid place-items-center"
      style={{
        width: size,
        height: size,
        background: bg,
