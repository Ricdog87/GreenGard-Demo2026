'use client';

import Image from 'next/image';
import Link from 'next/link';
import { useEffect, useRef, useState } from 'react';
import { gsap } from 'gsap';
import { ArrowDown, ArrowRight } from 'lucide-react';
import { MagneticButton } from '@/components/MagneticButton';
import { Button } from '@/components/ui/button';

// Pro Zeile wird ein Span pro Buchstabe erzeugt, damit GSAP einen Reveal animieren kann.
function HeroHeadline() {
  const lines: { text: string; italic?: boolean }[] = [
    { text: 'Der unsichtbare' },
    { text: 'Luxus eines' },
    { text: 'perfekten Gartens.', italic: true },
  ];

  return (
    <h1 className="hero-h text-linen mt-6 max-w-[15ch]">
      {lines.map((line, li) => (
        <span key={li} className="block overflow-hidden">
          <span className="inline-block">
            {line.text.split('').map((ch, i) =>
              ch === ' ' ? (
                <span key={`${li}-${i}`} className="hero-char inline-block">&nbsp;</span>
              ) : (
                <span
                  key={`${li}-${i}`}
                  className={`hero-char inline-block ${line.italic ? 'italic' : ''}`}
                >
                  {ch}
                </span>
              )
            )}
          </span>
        </span>
      ))}
    </h1>
  );
}

function LiveStamp() {
  const [now, setNow] = useState<string>('');
  useEffect(() => {
    const fmt = () => {
      const d = new Date();
      const t = d.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });
      setNow(t);
    };
    fmt();
    const i = setInterval(fmt, 30_000);
    return () => clearInterval(i);
  }, []);
  return (
    <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-linen/70 num">
      Wiesbaden · {now || '—'} · 22°C · Sonnig
    </div>
  );
}

export function Hero() {
  const containerRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    if (!containerRef.current) return;
    const ctx = gsap.context(() => {
      gsap.set('.hero-char', { yPercent: 110 });
      gsap.set('.hero-reveal', { opacity: 0, y: 18 });
      const tl = gsap.timeline({ delay: 1.6 });
      tl.to('.hero-char', {
        yPercent: 0,
        stagger: 0.018,
        duration: 0.9,
        ease: 'power3.out',
      });
      tl.to(
        '.hero-reveal',
        { opacity: 1, y: 0, stagger: 0.08, duration: 0.7, ease: 'power3.out' },
        '-=0.5'
      );
    }, containerRef);
    return () => ctx.revert();
  }, []);

  return (
    <section
      ref={containerRef}
      className="relative h-[100svh] min-h-[640px] w-full overflow-hidden bg-forest text-linen"
    >
      <Image
        src="https://images.unsplash.com/photo-1558904541-efa843a96f01?auto=format&fit=crop&w=2400&q=85"
        alt="Garten im Goldlicht"
        fill
        priority
        sizes="100vw"
        className="object-cover animate-kenburns"
        data-cursor="view"
      />
      <div className="absolute inset-0 bg-gradient-to-t from-forest/85 via-forest/30 to-transparent" />
      <div className="absolute inset-0 bg-gradient-to-r from-forest/45 via-transparent to-transparent" />

      <div className="absolute inset-0 z-10 container flex flex-col justify-between py-8 pt-6 md:py-12">
        <div className="flex items-start justify-between gap-6">
          <p className="hero-reveal font-mono text-[10px] uppercase tracking-[0.22em] text-linen/80">
            Est. 2004 · Wiesbaden
          </p>
          <p className="hero-reveal hidden sm:block font-mono text-[10px] uppercase tracking-[0.22em] text-linen/70">
            Atelier für Smart-Garden-Architektur · MMXXVI
          </p>
        </div>

        <div className="flex-1 flex items-end pb-4">
          <div className="w-full">
            <HeroHeadline />
            <p className="hero-reveal mt-8 max-w-xl text-linen/80 text-base md:text-lg leading-snug">
              <span className="italic">Smart Irrigation. Engineered Lighting. Autonomous Mowing.</span> Seit zwei Jahrzehnten.
            </p>
            <div className="hero-reveal mt-10 flex flex-wrap items-center gap-4">
              <MagneticButton>
                <Button asChild variant="accent" size="xl">
                  <Link href="/planung">
                    Planung beginnen <ArrowRight className="h-4 w-4" />
                  </Link>
                </Button>
              </MagneticButton>
              <Button asChild variant="ghost" size="xl" className="text-linen hover:bg-linen/10">
                <Link href="/kollektion">Kollektion entdecken</Link>
              </Button>
            </div>
          </div>
        </div>

        <div className="flex items-end justify-between gap-6">
          <div className="hero-reveal flex items-center gap-3 text-linen/80">
            <span className="font-mono text-[10px] uppercase tracking-[0.22em]">Scroll</span>
            <span className="block h-px w-12 bg-linen/40 relative overflow-hidden">
              <span className="absolute inset-y-0 left-0 w-1/3 bg-linen animate-[marquee_2s_linear_infinite]" />
            </span>
            <ArrowDown className="h-3.5 w-3.5" />
          </div>
          <div className="hero-reveal">
            <LiveStamp />
          </div>
        </div>
      </div>
    </section>
  );
}
