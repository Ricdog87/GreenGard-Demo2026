        ...
        <SheetHeader>
          ...
          <SheetTitle>
            {items.length === 0
              ...
              : `${items.length} ${items.length === 1 ? 'Schulung' : 'Schulungen'}`}
          </SheetTitle>
        </SheetHeader>
          ...
          {items.map((l) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                {l.termin && (
                  <p className="num font-mono mt-1 text-[10px] uppercase tracking-[0.16em] text-bronze">
                    Termin {l.termin}
                  </p>
                )}
                <div className="mt-3 inline-flex items-center border border-mist">
                  <button
                    ...
                    className="grid h-8 w-8 place-items-center hover:bg-linen"
                                        aria-label="Teilnehmerzahl verringern"
                  >
                  ...
                  <button
                    ...
                    onClick={() => setQty(l.slug, l.qty + 1)}
                    className="grid h-8 w-8 place-items-center hover:bg-linen"
                                        aria-label="Teilnehmerzahl erhöhen"
                  >
        ...
        {items.length > 0 && (
          <div className="space-y-5 border-t border-mist bg-linen/40 px-7 py-6">
            <div className="flex items-baseline justify-between">
              ...
              {priceLabel(mode)} · je Teilnehmer
            </p>
            <SheetClose asChild>
              <Button asChild variant="primary" size="lg" className="w-full">
                <Link href="/checkout">Buchung abschließen →</Link>
              </Button>
            </SheetClose>
          </div>
        )}
